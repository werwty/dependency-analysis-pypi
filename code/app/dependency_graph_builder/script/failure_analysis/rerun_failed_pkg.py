...
def dependency_scanner(avail_pkg_list, start=0, end=-1, output_folder="."):
    ...
    output_folder_log_folder_failed = os.path.join(output_folder_log_folder, "failed")
    ...
    os.makedirs(output_folder_log_folder_failed, exist_ok=True)
    ...
    def log_fail(_pkg_orig_name, _reason):
        with open(os.path.join(output_folder_log_folder_failed, _pkg_orig_name), "w") as fp:
            fp.write(_reason)
...
def load_rerun_list():
    ret_val = []
    with open("rerun_list.txt", "r") as fp:
        for line in fp:
...
def main():
    ...
    avail_pkg_list = load_rerun_list()
    # avail_pkg_list = ['QUANTAXIS-CRAWLY']

    dependency_scanner(avail_pkg_list, 0, -1, "output/debug_output")
